...
function save_payment(){
	...
	$.post('/weblet/shopping/account_action/savePayment',$.param(data) ,function(data, status,jqXHR) {
...
function save_order(){
	...
	$.post('/weblet/shopping/account_action/createOrder',$.param(data) ,function(data, status,jqXHR) {
		var json = $.parseJSON(data);
		var error = json.error;		
		if (error == 'success') {
			window.location = '/tpl/payment_gateway?orderId=' + json.order;
		} else {
			show_alert('failed!');
		}
	});
